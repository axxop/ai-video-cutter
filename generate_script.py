...
2. 时间标记表示该段的说话时长（按 4 字/秒计算，100字≈25秒）
3. **行号必须是单个连续范围**，格式 [1-50]，不要用 [1-3,5-7] 这种多段格式
4. **每段 80-120 字**，时长 20-30 秒，不要太短
5. **尽量多分段**，至少 15-25 段，每段覆盖一个小情节或知识点
6. 开场要有吸引力，设置悬念
7. 故事主线清晰，节奏紧凑
8. 结尾要有升华或收束感
"""
...
**示例格式：**
```
[25s] [1-50] 开场内容，制造悬念，约100字，从第1到50行整合...
[28s] [51-110] 核心卖点，强化冲突，约112字，从第51到110行重组...
[30s] [111-180] 故事推进，设置反转，约120字，从第111到180行提炼...
```

**重要提醒：**
- 行号范围必须是单个连续的 [开始-结束]，不要出现逗号
- 每段时长要充足（20-30秒），内容要完整
- 多分段，让解说节奏更好
